...
R_Not_Pressed:
												...
												mov lockObjectStartWork, 0

												xor r10, r10
												mov r11d, BeesPerThread
												...
												;wait for worker threads to finish frame
												LocalCall WaitForLock
...
; UpdateThreadWork                                                                                                        -
;                                                                                                                      -
; Runs the update logic that is multythreaded
;-----------------------------------------------------------------------------------------------------------------------
;                                                                                                                      -
; In:  rcx, pointer to threadId                                                                                                         -
; Out: nothing                                                                                                                        -
; 
;-----------------------------------------------------------------------------------------------------------------------

UpdateThreadWork								proc                                                                  ; Declare function
;------[Local Data]-----------------------------------------------------------------------------------------------------
												local               holder:qword                                      ;
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												mov rax, qword ptr [rcx] ;threadId												
												mov ebx, BeesPerThread
												mul ebx
												mov r10, rax ;bee start index
												mov r11, r10
												add r11d, BeesPerThread ; bee end index

												;check if last thread and if so add extra bees to end index
												mov al, threadCount
												dec rax
												cmp rax, rcx
												jne Worker_loop
												;add extra bees
												add r11d, BeesPerThreadExtra
Worker_loop:
												bt lockObjectStartWork, 0
												jc Worker_loop
...
UpdateThreadWork									endp                                                                  ; End function
...
; WaitForLock                                                                                                        -
;                                                                                                                      -
; Waits for the lock for the worker threads, if this is the last thread calling it will release it
;-----------------------------------------------------------------------------------------------------------------------
;                                                                                                                      -
; In:  nothing                                                                                                         -
; Out: nothing                                                                                                                        -
...
WaitForLock										proc                                                                  ; Declare function
;------[Local Data]-----------------------------------------------------------------------------------------------------
												local               holder:qword                                      ;
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												lock inc finishedThreads
												mov al, finishedThreads
												cmp al, threadCount
												jl Spin_Lock

												mov lockObjectStartWork, 1
												mov lockObject, 0
												jmp Return 
