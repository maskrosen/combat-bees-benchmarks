...
;----- [Fix the WinDbg bug] --------------------------------------------------------------------------------------------
												...
												mov rax, starting_bees_per_team
												mov bl, threadCount
												div ebx
												mov BeesPerThread, eax
												mov BeesPerThreadExtra, edx
...
Create_Threads_Loop:
												dec edi
												cmp edi, 1 ;we reserve 1 thread for the main thread
												jl Create_Threads_Loop_End

												lea r12, threadIds
												mov qword ptr [r12 + rdi * sizeof(qword)], rdi

												xor rcx, rcx ;lpThreadAttributes
												xor rdx, rdx ;dwStackSize
												mov r8, UpdateThreadWork ;lpStartAddress
												lea r9, qword ptr [r12 + rdi * sizeof(qword)] ;lpParameter
												WinCall CreateThread, rcx, rdx, r8, r9, 0, 0,

												jmp Create_Threads_Loop
Create_Threads_Loop_End:
