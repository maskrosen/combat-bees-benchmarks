...
;-----------------------------------------------------------------------------------------------------------------------
;																													-
; In:  rcx teamIndex, rdx beeIndexStart, r8 beeEndIndex																										-
; Out: nothing																														-
; 
;-----------------------------------------------------------------------------------------------------------------------
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												mov rdi, rdx ;bee index
												mov r9, r8 ;bee end index
												;keep teamIndex in r8
												mov r8, rcx
												...
												cmp r9d, esi
												cmovl esi, r9d ;min of alive bees and bee end index
												...
												mul ebx
												mov r10, rax ;movement array offset

												mov rax, rdi
												mov rbx, sizeof(Vector3)
												mul ebx
												mov r13, rax ;rotation array offset
...
;-----------------------------------------------------------------------------------------------------------------------
;																													-
; In:  rcx, teamIndex, rdx bee start index, r8 bee end index 																										-
; Out: nothing																														-
; 
;-----------------------------------------------------------------------------------------------------------------------
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												cmp r9d, esi
												cmovl esi, r9d ;min of active bees and end index
...
;-----------------------------------------------------------------------------------------------------------------------
;                                                                                                                      -
; In:  rcx, teamIndex, rdx enemyTeamIndex, r8 bee start index, r9 bee end index                                                                                                      -
; Out: nothing                                                                                                                        -
; 
;-----------------------------------------------------------------------------------------------------------------------
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												;save bee end index on the stack
												push r10
...
AttackTarget_Bit_Loop:												
												...
												mul rbx
												add rax, r11 ;bee index
												cmp rax, qword ptr [rsp + sizeof(qword)] ;alivebeesCount pushed on stack
												jge AttackTarget_Loop_Mask_End
												cmp rax, qword ptr [rsp] ;bee start index pushed on stack
												jl AttackTarget_Bit_Loop_Continue
												mov r11, rax
												mov rbx, sizeof(dword)
												mul rbx 
...
AttackTarget_Loop_Mask_End:
												...
												pop rsi ;pop pushed bee start index from start
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												cmp r10d, esi
												cmovl esi, r10d ;min of bee end index and active bee count
...
;-----------------------------------------------------------------------------------------------------------------------
;																													-
; In:  rcx team index, rdx bee index start, r8 bee end index																												-
; Out: nothing																														-
; 
;-----------------------------------------------------------------------------------------------------------------------
...
;------[Save incoming registers]----------------------------------------------------------------------------------------
												...
												add esi, edi
												cmp r10d, esi
												cmovl esi, r10d ;min of bee end index and active bees

												cmp r9d, edi
												cmovg edi, r9d ;max of bee start index and dead start index
